...
def create_security_operations_gate(mats):
    ...
    cube("SecurityOps_ServiceCourt", group, (0, 0.08, 0.1), (17.6, 0.16, 10.8), mats["stone_shadow"], bevel=0.055)
    cube("SecurityOps_OperationsHall", group, (-1.55, 2.06, 0.92), (8.8, 3.8, 4.72), mats["dark"], bevel=0.052)
    cube("SecurityOps_IncidentResponseHall", group, (1.9, 2.0, -1.06), (7.6, 3.55, 3.16), mats["stone_shadow"], bevel=0.048)
    cube("SecurityOps_CommandFloor", group, (-1.35, 4.32, 0.78), (7.5, 1.48, 3.68), mats["dark"], bevel=0.038)
    cube("SecurityOps_CommandDeck", group, (-1.35, 5.28, 0.72), (6.25, 0.54, 3.2), mats["rubber"], bevel=0.03)
    cube("SecurityOps_GlassWarRoom", group, (4.45, 1.82, 0.24), (3.05, 3.26, 4.16), mats["glass"], bevel=0.035)
    cube("SecurityOps_GlassCommandStack", group, (4.42, 4.22, 0.22), (2.32, 2.12, 3.3), mats["glass"], bevel=0.028)
    ...
    cube("SecurityOps_RouteShieldAtrium", group, (0.1, 2.44, -4.5), (8.1, 4.1, 0.34), mats["glass"], bevel=0.026)
    cube("SecurityOps_ShieldGateLeft", group, (-3.85, 2.52, -4.38), (0.46, 4.35, 0.42), mats["dark"], bevel=0.018)
    cube("SecurityOps_ShieldGateRight", group, (4.05, 2.52, -4.38), (0.46, 4.35, 0.42), mats["dark"], bevel=0.018)
    cube("SecurityOps_ShieldGateBeam", group, (0.1, 4.56, -4.38), (8.35, 0.46, 0.48), mats["dark"], bevel=0.026)
    cube("SecurityOps_ShieldGlass", group, (0.1, 2.44, -4.62), (6.3, 2.92, 0.08), mats["glass"], bevel=0.008)
    cube("SecurityOps_ShieldGateLeftFin", group, (-4.48, 2.55, -4.34), (0.22, 3.82, 0.86), mats["screen"], rot=(0, 0, -0.14), bevel=0.01)
    cube("SecurityOps_ShieldGateRightFin", group, (4.68, 2.55, -4.34), (0.22, 3.82, 0.86), mats["screen"], rot=(0, 0, 0.14), bevel=0.01)
    cube("SecurityOps_ScanThreshold", group, (0.1, 0.2, -4.95), (6.8, 0.08, 0.8), mats["pink"], bevel=0.006)
    cube("SecurityOps_EntryCanopy", group, (0.35, 2.46, -3.28), (6.8, 0.34, 1.1), mats["dark"], bevel=0.024)
    ...
    cube("SecurityOps_BlueTeamBridge", group, (0.8, 4.92, 2.9), (8.8, 0.2, 0.3), mats["screen"], bevel=0.008)
    cube("SecurityOps_ScannerBridge", group, (0.45, 4.98, -2.78), (8.9, 0.24, 0.34), mats["pink"], bevel=0.01)
    cube("SecurityOps_ScannerBridgeLower", group, (0.45, 4.62, -2.72), (7.7, 0.12, 0.24), mats["screen"], bevel=0.008)
    cube("SecurityOps_CommandCatwalk", group, (0.6, 5.44, 2.72), (9.4, 0.16, 0.26), mats["mint"], bevel=0.008)
    cube("SecurityOps_CommandDeckRailFront", group, (-1.35, 5.62, -1.0), (6.4, 0.14, 0.12), mats["screen"], bevel=0.006)
    cube("SecurityOps_CommandDeckRailBack", group, (-1.35, 5.62, 2.38), (6.4, 0.14, 0.12), mats["mint"], bevel=0.006)
    cube("SecurityOps_RearOpsWindowBand", group, (-1.25, 2.78, 3.35), (6.8, 1.28, 0.08), mats["glass"], bevel=0.008)
    cube("SecurityOps_SideShieldWallLeft", group, (-5.38, 2.44, -1.0), (0.08, 2.58, 3.5), mats["screen"], bevel=0.006)
    cube("SecurityOps_SideShieldWallRight", group, (5.86, 2.44, -1.0), (0.08, 2.58, 3.5), mats["mint"], bevel=0.006)
...
def create_sentinel_soc_tower(mats):
    ...
    cube("SentinelSoc_OperationsHall", group, (-2.55, 1.7, 0.75), (9.7, 3.14, 5.7), mats["dark"], bevel=0.052)
    cube("SentinelSoc_IncidentResponseHall", group, (0.9, 2.05, -1.05), (7.5, 3.55, 3.25), mats["stone_shadow"], bevel=0.048)
    cube("SentinelSoc_UpperOperationsDeck", group, (-2.18, 4.02, 0.68), (8.9, 1.72, 4.68), mats["dark"], bevel=0.04)
    cube("SentinelSoc_OverwatchDeck", group, (-2.1, 5.2, 0.48), (7.1, 0.54, 4.2), mats["rubber"], bevel=0.03)
    cube("SentinelSoc_SolidCommandWing", group, (3.95, 1.72, -0.25), (3.85, 2.95, 4.55), mats["stone_shadow"], bevel=0.042)
    cube("SentinelSoc_CommandWindowBand", group, (3.95, 2.0, -2.58), (3.12, 0.92, 0.08), mats["screen"], bevel=0.008)
    cube("SentinelSoc_CommandSideWindow", group, (5.92, 1.9, -0.16), (0.08, 1.65, 2.92), mats["glass"], bevel=0.008)
    cube("SentinelSoc_CommandTower", group, (3.88, 3.82, -0.25), (2.82, 3.5, 3.75), mats["dark"], bevel=0.034)
    cube("SentinelSoc_CommandTowerGlass", group, (3.88, 4.18, -2.18), (2.1, 1.86, 0.08), mats["glass"], bevel=0.008)
    cube("SentinelSoc_CommandCrownBar", group, (3.88, 5.84, -0.25), (3.1, 0.28, 3.95), mats["screen"], bevel=0.012)
    cube("SentinelSoc_TowerCore", group, (-6.55, 3.02, 0.8), (2.15, 5.6, 2.05), mats["stone_shadow"], bevel=0.04)
    cube("SentinelSoc_TowerSpireCore", group, (-6.55, 6.82, 0.8), (1.35, 4.2, 1.35), mats["stone_shadow"], bevel=0.03)
    cube("SentinelSoc_TowerCrown", group, (-6.55, 5.98, 0.8), (3.2, 0.58, 2.8), mats["dark"], bevel=0.035)
    cube("SentinelSoc_OverwatchCrown", group, (-6.55, 9.05, 0.8), (2.35, 0.34, 2.08), mats["dark"], bevel=0.026)
    cube("SentinelSoc_CrownScanLine", group, (-6.55, 6.18, -0.76), (2.55, 0.1, 0.08), mats["pink"], bevel=0.006)
    cube("SentinelSoc_OverwatchScanLine", group, (-6.55, 9.2, -0.42), (1.75, 0.08, 0.07), mats["pink"], bevel=0.004)
    ...
    cube("SentinelSoc_AlertWall", group, (0.45, 2.12, -2.45), (6.9, 2.34, 0.2), mats["rubber"], bevel=0.028)
    cube("SentinelSoc_AlertScreen", group, (0.45, 2.17, -2.6), (5.78, 1.58, 0.08), mats["pink"], bevel=0.006)
    cube("SentinelSoc_RouteShieldAtrium", group, (1.3, 2.28, -4.74), (7.85, 4.08, 0.42), mats["stone_shadow"], bevel=0.03)
    cube("SentinelSoc_RouteShieldGlassSlot", group, (1.3, 2.28, -4.98), (5.7, 2.55, 0.08), mats["glass"], bevel=0.008)
    cube("SentinelSoc_RouteShieldHeader", group, (1.3, 4.16, -5.02), (7.35, 0.28, 0.1), mats["screen"], bevel=0.006)
    cube("SentinelSoc_ShieldGateFrame", group, (1.3, 2.36, -4.95), (7.95, 4.1, 0.22), mats["dark"], bevel=0.026)
    cube("SentinelSoc_ShieldGateGlass", group, (1.3, 2.18, -5.08), (5.9, 2.82, 0.08), mats["glass"], bevel=0.008)
    cube("SentinelSoc_ShieldGateLeftFin", group, (-3.0, 2.34, -4.92), (0.22, 3.65, 0.9), mats["screen"], rot=(0, 0, -0.12), bevel=0.01)
    cube("SentinelSoc_ShieldGateRightFin", group, (5.62, 2.34, -4.92), (0.22, 3.65, 0.9), mats["screen"], rot=(0, 0, 0.12), bevel=0.01)
    cube("SentinelSoc_ScanThreshold", group, (1.3, 0.22, -5.08), (5.8, 0.08, 0.72), mats["pink"], bevel=0.006)
    cube("SentinelSoc_ScanPortalLeftBase", group, (-4.12, 0.72, -5.05), (0.52, 1.24, 0.62), mats["dark"], bevel=0.018)
    cube("SentinelSoc_ScanPortalRightBase", group, (6.72, 0.72, -5.05), (0.52, 1.24, 0.62), mats["dark"], bevel=0.018)
    cube("SentinelSoc_ScanPortalBeam", group, (1.3, 3.32, -5.12), (11.25, 0.24, 0.28), mats["dark"], bevel=0.018)
    cube("SentinelSoc_ScanPortalBeamGlow", group, (1.3, 3.52, -5.3), (8.7, 0.08, 0.06), mats["pink"], bevel=0.004)
    ...
    cube("SentinelSoc_ThreatIntelMast", group, (-3.9, 5.22, 2.65), (0.42, 4.45, 0.42), mats["dark"], bevel=0.018)
    cube("SentinelSoc_ThreatIntelSpire", group, (-3.9, 8.05, 2.65), (0.22, 3.4, 0.22), mats["dark"], bevel=0.008)
    cube("SentinelSoc_ThreatIntelArray", group, (-3.9, 6.95, 2.65), (3.1, 0.18, 0.18), mats["pink"], bevel=0.008)
    cube("SentinelSoc_ThreatIntelUpperArray", group, (-3.9, 8.72, 2.65), (2.4, 0.12, 0.12), mats["screen"], bevel=0.006)
    cube("SentinelSoc_ThreatWallBase", group, (-1.95, 1.6, 3.58), (6.8, 2.55, 0.22), mats["dark"], bevel=0.024)
    cube("SentinelSoc_ThreatWallMap", group, (-1.95, 1.72, 3.72), (5.55, 1.55, 0.08), mats["pink"], bevel=0.006)
    cube("SentinelSoc_ThreatWallStatusRail", group, (-1.95, 2.72, 3.78), (6.2, 0.12, 0.08), mats["screen"], bevel=0.004)
    ...
    cube("SentinelSoc_BlueTeamBridge", group, (0.55, 4.38, 2.82), (8.4, 0.22, 0.34), mats["screen"], bevel=0.01)
    cube("SentinelSoc_ScannerBridge", group, (0.28, 4.9, -2.72), (8.6, 0.26, 0.34), mats["pink"], bevel=0.01)
    cube("SentinelSoc_ScannerBridgeLower", group, (0.28, 4.55, -2.68), (7.6, 0.12, 0.24), mats["screen"], bevel=0.008)
    cube("SentinelSoc_CommandCatwalk", group, (0.2, 5.22, 2.66), (9.4, 0.16, 0.28), mats["mint"], bevel=0.008)
    cube("SentinelSoc_OverwatchRailFront", group, (-2.1, 5.56, -1.48), (7.3, 0.16, 0.12), mats["screen"], bevel=0.006)
    cube("SentinelSoc_OverwatchRailBack", group, (-2.1, 5.56, 2.5), (7.3, 0.16, 0.12), mats["mint"], bevel=0.006)
