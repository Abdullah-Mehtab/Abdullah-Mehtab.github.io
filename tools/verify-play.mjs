...
try {
  ...
  const gate4frCSentinelInspection = await captureGate4FRCSentinelInspection(page);
...
async function captureGate4FRCCareerInspection(page) {
  ...
  const views = [
    { id: 'front', right: 0, forward: -28, y: 11.6, lookY: 4.1, fov: 49 },
    { id: 'rear', right: 0, forward: 24, y: 10.2, lookY: 4.0, fov: 50 },
    { id: 'left', right: -25, forward: 0, y: 10.2, lookY: 4.0, fov: 50 },
    { id: 'right', right: 25, forward: 0, y: 10.2, lookY: 4.0, fov: 50 },
    { id: 'close', right: 0, forward: -14.8, y: 5.4, lookY: 3.0, fov: 54 },
    { id: 'top', right: 0, forward: 0, y: 40, lookY: 0.1, fov: 42 },
    { id: 'ui-hidden-concept-read', right: 0, forward: -26, y: 10.2, lookY: 3.8, fov: 49, hideUi: true }
  ];
  const screenshots = [];
  ...
  for (const view of views) {
    const sample = await page.evaluate((viewSpec) => {
      const game = window.__portfolioDrive.game;
      const Vector3 = game.camera.position.constructor;
      const anchor = { x: 74, z: -60, rotation: -0.65 };
      const localPoint = (right, forward, y = 0) =>
        new Vector3(
          anchor.x + Math.cos(anchor.rotation) * right + Math.sin(anchor.rotation) * forward,
          y,
      ...
      const vehiclePoint = localPoint(0, -17.2, 1.08);
      const direction = localPoint(0, 0, 0).sub(vehiclePoint).setY(0).normalize();
      const heading = Math.atan2(direction.x, direction.z);
      game.ui.closePanel?.();
      game.ui.closeMap?.();
...
async function captureGate4FRCContactInspection(page) {
  ...
  const views = [
    { id: 'front', right: -9, forward: 31, y: 11.4, lookY: 4.1, fov: 51 },
    { id: 'rear', right: 0, forward: -25, y: 10.2, lookY: 4.0, fov: 50 },
    { id: 'left', right: -27, forward: 3, y: 10.2, lookY: 4.0, fov: 50 },
    { id: 'right', right: 27, forward: 3, y: 10.2, lookY: 4.0, fov: 50 },
    { id: 'close', right: -7, forward: 17, y: 5.5, lookY: 3.0, fov: 54 },
    { id: 'top', right: 0, forward: 0, y: 42, lookY: 0.1, fov: 42 },
    { id: 'ui-hidden-concept-read', right: -9, forward: 30, y: 10.2, lookY: 3.8, fov: 51, hideUi: true }
  ];
  const screenshots = [];
  ...
  for (const view of views) {
    const sample = await page.evaluate((viewSpec) => {
      const game = window.__portfolioDrive.game;
      const Vector3 = game.camera.position.constructor;
      const base = { x: 127, z: 13, rotation: -0.34 };
      const station = {
        x: base.x + Math.cos(base.rotation) * 1 + Math.sin(base.rotation) * 3.05,
        z: base.z - Math.sin(base.rotation) * 1 + Math.cos(base.rotation) * 3.05,
        rotation: base.rotation
      };
      const localPoint = (right, forward, y = 0) =>
        new Vector3(
          station.x + Math.cos(station.rotation) * right + Math.sin(station.rotation) * forward,
          y,
          station.z - Math.sin(station.rotation) * right + Math.cos(station.rotation) * forward
        );

      const vehiclePoint = localPoint(-11.5, 22, 1.08);
      const direction = localPoint(0, 0, 0).sub(vehiclePoint).setY(0).normalize();
      const heading = Math.atan2(direction.x, direction.z);
      game.ui.closePanel?.();
      game.ui.closeMap?.();
...
async function captureGate4FRCSentinelInspection(page) {
  ...
  const views = [
    { id: 'front', right: 0, forward: -30, y: 13.2, lookY: 5.0, fov: 50 },
    { id: 'rear', right: 0, forward: 27, y: 11.4, lookY: 4.6, fov: 51 },
    { id: 'left', right: -29, forward: 0, y: 11.4, lookY: 4.6, fov: 51 },
    { id: 'right', right: 29, forward: 0, y: 11.4, lookY: 4.6, fov: 51 },
    { id: 'close', right: 0, forward: -16, y: 6.2, lookY: 3.4, fov: 55 },
    { id: 'top', right: 0, forward: 0, y: 47, lookY: 0.1, fov: 42 },
    { id: 'ui-hidden-concept-read', right: 0, forward: -28, y: 11.8, lookY: 4.6, fov: 50, hideUi: true }
  ];
  const screenshots = [];
  ...
  for (const view of views) {
    const sample = await page.evaluate((viewSpec) => {
      ...
      const vehiclePoint = localPoint(0, -20, 1.08);
      const direction = localPoint(0, 0, 0).sub(vehiclePoint).setY(0).normalize();
      const heading = Math.atan2(direction.x, direction.z);
      game.ui.closePanel?.();
      game.ui.closeMap?.();
    ...
    await delay(260);
    const name = `gate4fr-c-sentinel-${view.id}.png`;
    await screenshot(page, name);
    screenshots.push({ ...sample, name });
    if (view.hideUi) {
      await page.evaluate(() => {
...
async function captureGate4FRCAwardsInspection(page) {
  ...
  const views = [
    { id: 'front', right: 0, forward: -31, y: 12.0, lookY: 4.2, fov: 50 },
    { id: 'rear', right: 0, forward: 27, y: 10.8, lookY: 4.0, fov: 51 },
    { id: 'left', right: -28, forward: 0, y: 10.8, lookY: 4.0, fov: 51 },
    { id: 'right', right: 28, forward: 0, y: 10.8, lookY: 4.0, fov: 51 },
    { id: 'close', right: 0, forward: -15, y: 6.4, lookY: 3.2, fov: 55 },
    { id: 'top', right: 0, forward: 0, y: 44, lookY: 0.1, fov: 42 },
    { id: 'ui-hidden-concept-read', right: 0, forward: -30, y: 11.2, lookY: 4.0, fov: 50, hideUi: true }
  ];
  const screenshots = [];
  ...
  for (const view of views) {
    ...
    await delay(260);
    const name = `gate4fr-c-awards-${view.id}.png`;
    await screenshot(page, name);
    screenshots.push({ ...sample, name });
    if (view.hideUi) {
      await page.evaluate(() => {
...
async function captureGate4FRCTodoInspection(page) {
  ...
  const views = [
    { id: 'front', right: 0, forward: -29, y: 11.2, lookY: 3.8, fov: 50 },
    { id: 'rear', right: 0, forward: 25, y: 10.4, lookY: 3.6, fov: 51 },
    { id: 'left', right: -27, forward: 0, y: 10.4, lookY: 3.6, fov: 51 },
    { id: 'right', right: 27, forward: 0, y: 10.4, lookY: 3.6, fov: 51 },
    { id: 'close', right: 0, forward: -13, y: 5.9, lookY: 2.8, fov: 55 },
    { id: 'top', right: 0, forward: 0, y: 42, lookY: 0.1, fov: 42 },
    { id: 'ui-hidden-concept-read', right: 0, forward: -29, y: 10.8, lookY: 3.8, fov: 50, hideUi: true }
  ];
  const screenshots = [];
  ...
  for (const view of views) {
    const sample = await page.evaluate((viewSpec) => {
      const game = window.__portfolioDrive.game;
      const Vector3 = game.camera.position.constructor;
      const anchor = { x: -96, z: 4, rotation: 0.24 };
      const localPoint = (right, forward, y = 0) =>
        new Vector3(
          anchor.x + Math.cos(anchor.rotation) * right + Math.sin(anchor.rotation) * forward,
          y,
    ...
    await delay(260);
    const name = `gate4fr-c-todo-${view.id}.png`;
    await screenshot(page, name);
    screenshots.push({ ...sample, name });
    if (view.hideUi) {
      await page.evaluate(() => {
...
async function captureGate4FRCPotatoInspection(page) {
  ...
  const views = [
    { id: 'front', right: 0, forward: -27, y: 9.8, lookY: 2.6, fov: 50 },
    { id: 'rear', right: 0, forward: 24, y: 9.2, lookY: 2.4, fov: 51 },
    { id: 'left', right: -24, forward: 0, y: 8.8, lookY: 2.4, fov: 51 },
    { id: 'right', right: 24, forward: 0, y: 8.8, lookY: 2.4, fov: 51 },
    { id: 'close', right: 0, forward: -12.5, y: 5.0, lookY: 1.8, fov: 55 },
    { id: 'top', right: 0, forward: 0, y: 39, lookY: 0.1, fov: 42 },
    { id: 'ui-hidden-concept-read', right: 0, forward: -27, y: 9.6, lookY: 2.6, fov: 50, hideUi: true }
  ];
  const screenshots = [];
  ...
  for (const view of views) {
    const sample = await page.evaluate((viewSpec) => {
      ...
      const respawn = game.world.getRespawnPose('potato');
      const vehiclePoint = respawn.position.clone();
      const direction = localPoint(0, 0, 0).sub(vehiclePoint).setY(0).normalize();
      const heading = Math.atan2(direction.x, direction.z);
      game.ui.closePanel?.();
      game.ui.closeMap?.();
    ...
    await delay(260);
    const name = `gate4fr-c-potato-${view.id}.png`;
    await screenshot(page, name);
    screenshots.push({ ...sample, name });
    if (view.hideUi) {
      await page.evaluate(() => {
...
async function captureMobile(browser) {
  ...
  if (await isGate4FRCInspectionGate(page)) {
    ...
    await stageRouteApproachView(page, 'sentinel');
    await delay(350);
    await page.screenshot({ path: join(outputDir, 'mobile-sentinel.png'), fullPage: true });
    await stageRouteApproachView(page, 'awards');
    await delay(350);
    await page.screenshot({ path: join(outputDir, 'mobile-awards.png'), fullPage: true });
    await stageRouteApproachView(page, 'todo');
    await delay(350);
    await page.screenshot({ path: join(outputDir, 'mobile-todo.png'), fullPage: true });
    await stageRouteApproachView(page, 'potato');
    await delay(350);
    await page.screenshot({ path: join(outputDir, 'mobile-potato.png'), fullPage: true });
  }
  const sample = await page.evaluate(async () => {
    const game = window.__portfolioDrive.game;
    game.renderer.info.reset();
...
function assertVerification(result) {
  ...
  if (result.goalGate === 'gate-4e-bn-projects-release-foundry-drive-by-pass') {
    assertGate4EBNProjectsReleaseFoundryDriveByVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-bo-career-campus-route-framing-pass') {
    assertGate4EBOCareerCampusRouteFramingVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  ...
  if (result.goalGate === 'gate-4e-bt-route-place-integration-pass') {
    assertGate4EBTRoutePlaceIntegrationVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-bu-contact-route-exchange-benchmark-pass') {
    assertGate4EBUContactRouteExchangeBenchmarkVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-bv-cv-route-side-archive-readability-pass') {
    assertGate4EBVCvRouteSideArchiveReadabilityVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-bw-todo-route-sprint-board-benchmark-pass') {
    assertGate4EBWTodoRouteSprintBoardBenchmarkVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-bx-awards-route-honors-forecourt-benchmark-pass') {
    assertGate4EBXAwardsRouteHonorsForecourtBenchmarkVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-by-security-scanner-threshold-benchmark-pass') {
  ...
  if (result.goalGate === 'gate-4e-ca-projects-route-foundry-facade-benchmark-pass') {
    assertGate4ECAProjectsRouteFoundryFacadeBenchmarkVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-cb-skills-route-learning-systems-facade-benchmark-pass') {
  ...
  if (result.goalGate === 'gate-4e-ce-launch-hub-forecourt-first-frame-benchmark-pass') {
  ...
  if (result.goalGate === 'gate-4e-ci-security-operations-threshold-benchmark-pass') {
    assertGate4ECISecurityOperationsThresholdBenchmarkVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-cj-circuit-drive-through-checkpoint-benchmark-pass') {
    assertGate4ECJCircuitDriveThroughCheckpointBenchmarkVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-ck-awards-route-honors-arrival-benchmark-pass') {
    assertGate4ECKAwardsRouteHonorsArrivalBenchmarkVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-cl-launch-hub-world-entry-first-frame-benchmark-pass') {
  ...
  if (result.goalGate === 'gate-4e-cn-contact-public-signal-arrival-benchmark-pass') {
    assertGate4ECNContactPublicSignalArrivalBenchmarkVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-co-projects-hierarchy-simplification-benchmark-pass') {
    assertGate4ECOProjectsHierarchySimplificationBenchmarkVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-cp-projects-build-hall-silhouette-benchmark-pass') {
    assertGate4ECPProjectsBuildHallSilhouetteBenchmarkVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-cq-projects-single-hall-facade-benchmark-pass') {
    assertGate4ECQProjectsSingleHallFacadeBenchmarkVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-cr-projects-material-silhouette-benchmark-pass') {
    assertGate4ECRProjectsMaterialSilhouetteBenchmarkVerification(result, failures);
    if (failures.length) {
      throw new Error(`Play verification failed: ${failures.join('; ')}`);
    }
    return;
  }
  if (result.goalGate === 'gate-4e-cs-awards-solid-entry-benchmark-pass') {
...
function isGate4EExpandedArchitectureGate(goalGate) {
  return goalGate === 'gate-4e-c-monumental-scale-pass'
    ...
    || goalGate === 'gate-4e-bt-route-place-integration-pass'
    || goalGate === 'gate-4e-bu-contact-route-exchange-benchmark-pass'
    ...
    || goalGate === 'gate-4e-cn-contact-public-signal-arrival-benchmark-pass'
    || goalGate === 'gate-4e-co-projects-hierarchy-simplification-benchmark-pass'
...
function assertGate3RVerticalSliceVerification(result, failures, options = {}) {
  ...
  const allowProtectedFccExactAtDistance = result.goalGate === 'gate-4e-m-protected-fcc-visibility-pass'
    ...
    || result.goalGate === 'gate-4e-bt-route-place-integration-pass'
    || result.goalGate === 'gate-4e-bu-contact-route-exchange-benchmark-pass'
    ...
    || result.goalGate === 'gate-4e-cn-contact-public-signal-arrival-benchmark-pass'
    || result.goalGate === 'gate-4e-co-projects-hierarchy-simplification-benchmark-pass'
  ...
  const highQualityCallBudget = gate4eExpandedArchitecture ? 315 : 300;
  if ((result.highQuality?.calls || 0) > highQualityCallBudget) failures.push(`Gate 3R high quality draw-call budget exceeded: ${result.highQuality?.calls || 0}`);
...
function assertGate4EBMSecurityOperationsCampusArrivalVerification(result, failures, expectedGoal = 'gate-4e-bm-security-operations-campus-arrival-pass') {
  assertGate4EBLAwardsHonorsBoulevardVerification(result, failures, expectedGoal);
...
function assertGate4EBNProjectsReleaseFoundryDriveByVerification(result, failures, expectedGoal = 'gate-4e-bn-projects-release-foundry-drive-by-pass') {
  assertGate4EBMSecurityOperationsCampusArrivalVerification(result, failures, expectedGoal);

  const projects = result.gate4b4?.projects || {};
  assertAuthoredDistrictAsset(result, 'EnvPolishProjectsFoundryBuilding', 'Gate 4-E-BN Projects release foundry architecture', failures);
  if ((projects.routeReleaseGantryFrames || 0) < 1) failures.push(`Gate 4-E-BN Projects failed: routeReleaseGantryFrames=${projects.routeReleaseGantryFrames || 0}/1`);
  if ((projects.routePrototypeLaunchPortals || 0) < 1) failures.push(`Gate 4-E-BN Projects failed: routePrototypeLaunchPortals=${projects.routePrototypeLaunchPortals || 0}/1`);
  if ((projects.routeBuildReviewCourts || 0) < 1) failures.push(`Gate 4-E-BN Projects failed: routeBuildReviewCourts=${projects.routeBuildReviewCourts || 0}/1`);
  if ((projects.routeDeployBeaconStacks || 0) < 1) failures.push(`Gate 4-E-BN Projects failed: routeDeployBeaconStacks=${projects.routeDeployBeaconStacks || 0}/1`);
  if ((projects.routePublicShowcaseAtriums || 0) < 1) failures.push(`Gate 4-E-BN Projects carry-forward failed: routePublicShowcaseAtriums=${projects.routePublicShowcaseAtriums || 0}/1`);
  if ((projects.routeReleaseTheaterScreens || 0) < 1) failures.push(`Gate 4-E-BN Projects carry-forward failed: routeReleaseTheaterScreens=${projects.routeReleaseTheaterScreens || 0}/1`);
  if ((projects.routePrototypeGalleryPods || 0) < 3) failures.push(`Gate 4-E-BN Projects carry-forward failed: routePrototypeGalleryPods=${projects.routePrototypeGalleryPods || 0}/3`);
  if ((projects.signs || 0) !== 0) failures.push(`Gate 4-E-BN Projects failed: rejected signs=${projects.signs || 0}`);
  if ((projects.lamps || 0) !== 0) failures.push(`Gate 4-E-BN Projects failed: rejected lamps=${projects.lamps || 0}`);
  if ((result.colliderCount || 0) !== 2) failures.push(`Gate 4-E-BN failed: unexpected collider count=${result.colliderCount || 0}`);
...
function assertGate4EBOCareerCampusRouteFramingVerification(result, failures, expectedGoal = 'gate-4e-bo-career-campus-route-framing-pass') {
  ...
  if ((career.routeArrivalBoulevards || 0) < 2) failures.push(`Gate 4-E-BO Career failed: routeArrivalBoulevards=${career.routeArrivalBoulevards || 0}/2`);
  if ((career.routeReceptionCanopies || 0) < 1) failures.push(`Gate 4-E-BO Career failed: routeReceptionCanopies=${career.routeReceptionCanopies || 0}/1`);
  if ((career.routeHiringReviewPavilions || 0) < 2) failures.push(`Gate 4-E-BO Career failed: routeHiringReviewPavilions=${career.routeHiringReviewPavilions || 0}/2`);
  if ((career.routeTeamFlowBridges || 0) < 1) failures.push(`Gate 4-E-BO Career failed: routeTeamFlowBridges=${career.routeTeamFlowBridges || 0}/1`);
  if ((career.routeSprintBeaconStacks || 0) < 2) failures.push(`Gate 4-E-BO Career failed: routeSprintBeaconStacks=${career.routeSprintBeaconStacks || 0}/2`);
  if ((career.signs || 0) !== 0) failures.push(`Gate 4-E-BO Career failed: rejected signs=${career.signs || 0}`);
  if ((career.lamps || 0) !== 0) failures.push(`Gate 4-E-BO Career failed: rejected lamps=${career.lamps || 0}`);

  const careerPresentation = result.zonePresentation?.samples?.find((sample) => sample.id === 'career');
  if (!careerPresentation) {
    failures.push('Gate 4-E-BO Career failed: missing zone presentation sample');
  } else {
    if (careerPresentation.surface !== 'road') failures.push(`Gate 4-E-BO Career failed: respawn surface=${careerPresentation.surface}`);
    if (careerPresentation.cameraDistance < 28 || careerPresentation.cameraDistance > 42) failures.push(`Gate 4-E-BO Career failed: cameraDistance=${careerPresentation.cameraDistance}`);
    if (careerPresentation.targetDistance > 15.8) failures.push(`Gate 4-E-BO Career failed: targetDistance=${careerPresentation.targetDistance}`);
    if (careerPresentation.fov < 46 || careerPresentation.fov > 49) failures.push(`Gate 4-E-BO Career failed: fov=${careerPresentation.fov}`);
  }
...
function assertGate4EBSBrunoRouteApproachBenchmarkEvidenceVerification(result, failures, expectedGoal = 'gate-4e-bs-bruno-route-approach-benchmark-evidence') {
  assertGate4EBRVehicleFirstPresentationFramingVerification(result, failures, expectedGoal);
...
function assertGate4EBTRoutePlaceIntegrationVerification(result, failures, expectedGoal = 'gate-4e-bt-route-place-integration-pass') {
  assertGate4EBSBrunoRouteApproachBenchmarkEvidenceVerification(result, failures, expectedGoal);

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const required = [
    ['sentinel', 26, 'Gate 4-E-BT Sentinel'],
    ['skills', 23, 'Gate 4-E-BT Skills']
  ];

  for (const [zoneId, minApproachDistance, label] of required) {
    const frame = frames.get(zoneId);
    if (!frame) {
      failures.push(`${label} failed: missing route approach frame`);
      continue;
    }
    if (!frame.safeInFrame) failures.push(`${label} failed: safeInFrame=${frame.safeInFrame}`);
    if (!frame.vehicle?.safeInFrame) {
      failures.push(`${label} failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
    }
    if (!frame.landmark?.inFrame) {
      failures.push(`${label} failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
    }
    if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < minApproachDistance) {
      failures.push(`${label} failed: approachDistance=${frame.approachDistance}, min=${minApproachDistance}`);
    }
    if (!frame.road?.offsetSafe) {
      failures.push(`${label} failed: road lateral=${frame.road?.lateralOffset}`);
    }
  }
}

function assertGate4EBUContactRouteExchangeBenchmarkVerification(result, failures) {
  assertGate4EBTRoutePlaceIntegrationVerification(result, failures, 'gate-4e-bu-contact-route-exchange-benchmark-pass');

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('contact');
  if (!frame) {
    failures.push('Gate 4-E-BU Contact failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-BU Contact failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-BU Contact failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame) {
    failures.push(`Gate 4-E-BU Contact failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 24 || frame.approachDistance > 34) {
    failures.push(`Gate 4-E-BU Contact failed: approachDistance=${frame.approachDistance}`);
  }
  const lateral = frame.road?.lateralOffset;
  const halfWidth = frame.road?.roadHalfWidth;
  if (!Number.isFinite(lateral) || !Number.isFinite(halfWidth) || lateral > Math.max(0.8, halfWidth * 0.55)) {
    failures.push(`Gate 4-E-BU Contact failed: road-centered lateral=${lateral}, halfWidth=${halfWidth}`);
  }
}

function assertGate4EBVCvRouteSideArchiveReadabilityVerification(result, failures) {
  assertGate4EBUContactRouteExchangeBenchmarkVerification(
    { ...result, goalGate: 'gate-4e-bu-contact-route-exchange-benchmark-pass' },
    failures
  );

  const cv = result.gate4b1?.cv || {};
  if ((cv.routeSideArchiveFacades || 0) < 2) {
    failures.push(`Gate 4-E-BV CV failed: routeSideArchiveFacades=${cv.routeSideArchiveFacades || 0}/2`);
  }
  if ((cv.routeSideResumeSeals || 0) < 2) {
    failures.push(`Gate 4-E-BV CV failed: routeSideResumeSeals=${cv.routeSideResumeSeals || 0}/2`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('cv');
  if (!frame) {
    failures.push('Gate 4-E-BV CV failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-BV CV failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-BV CV failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame) {
    failures.push(`Gate 4-E-BV CV failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 16) {
    failures.push(`Gate 4-E-BV CV failed: approachDistance=${frame.approachDistance}`);
  }
  if (!frame.road?.offsetSafe) {
    failures.push(`Gate 4-E-BV CV failed: road lateral=${frame.road?.lateralOffset}`);
  }
}

function assertGate4EBWTodoRouteSprintBoardBenchmarkVerification(result, failures) {
  assertGate4EBVCvRouteSideArchiveReadabilityVerification(
    { ...result, goalGate: 'gate-4e-bv-cv-route-side-archive-readability-pass' },
    failures
  );
  ...
  if ((todo.routeSprintBoardFacades || 0) < 1) {
    failures.push(`Gate 4-E-BW Todo failed: routeSprintBoardFacades=${todo.routeSprintBoardFacades || 0}/1`);
  }
  if ((todo.routeChecklistSpines || 0) < 1) {
    failures.push(`Gate 4-E-BW Todo failed: routeChecklistSpines=${todo.routeChecklistSpines || 0}/1`);
  }
  if ((todo.routeCalendarCues || 0) < 1) {
    failures.push(`Gate 4-E-BW Todo failed: routeCalendarCues=${todo.routeCalendarCues || 0}/1`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('todo');
  if (!frame) {
    failures.push('Gate 4-E-BW Todo failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-BW Todo failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-BW Todo failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame) {
    failures.push(`Gate 4-E-BW Todo failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 16) {
    failures.push(`Gate 4-E-BW Todo failed: approachDistance=${frame.approachDistance}`);
  }
  if (!frame.road?.offsetSafe) {
    failures.push(`Gate 4-E-BW Todo failed: road lateral=${frame.road?.lateralOffset}`);
  }
}

function assertGate4EBXAwardsRouteHonorsForecourtBenchmarkVerification(result, failures) {
  assertGate4EBWTodoRouteSprintBoardBenchmarkVerification(result, failures);

  const awards = result.gate4b5?.awards || {};
  if ((awards.routeHonorsForecourts || 0) < 1) {
    failures.push(`Gate 4-E-BX Awards failed: routeHonorsForecourts=${awards.routeHonorsForecourts || 0}/1`);
  }
  if ((awards.routeCeremonialWalks || 0) < 1) {
    failures.push(`Gate 4-E-BX Awards failed: routeCeremonialWalks=${awards.routeCeremonialWalks || 0}/1`);
  }
  if ((awards.routeHonorsThresholds || 0) < 1) {
    failures.push(`Gate 4-E-BX Awards failed: routeHonorsThresholds=${awards.routeHonorsThresholds || 0}/1`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('awards');
  if (!frame) {
    failures.push('Gate 4-E-BX Awards failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-BX Awards failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-BX Awards failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-BX Awards failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 20 || frame.approachDistance > 46) {
    failures.push(`Gate 4-E-BX Awards failed: approachDistance=${frame.approachDistance}`);
  }
  if (!frame.road?.offsetSafe) {
    failures.push(`Gate 4-E-BX Awards failed: road lateral=${frame.road?.lateralOffset}`);
  }
}

function assertGate4EBYSecurityScannerThresholdBenchmarkVerification(result, failures) {
  assertGate4EBXAwardsRouteHonorsForecourtBenchmarkVerification(result, failures);

  if (!result.securityScan?.active?.active) failures.push('Gate 4-E-BY Security failed: scan active state not observed');
  if (!result.securityScan?.complete?.complete) failures.push('Gate 4-E-BY Security failed: scan complete state not observed');

  const security = result.securityLab || {};
  if ((security.operationsGates || 0) < 1) {
    failures.push(`Gate 4-E-BY Security failed: operationsGates=${security.operationsGates || 0}/1`);
  }
  if ((security.routeCampusArrivalArches || 0) < 1) {
    failures.push(`Gate 4-E-BY Security failed: routeCampusArrivalArches=${security.routeCampusArrivalArches || 0}/1`);
  }
  if ((security.routeScanCanopies || 0) < 1) {
    failures.push(`Gate 4-E-BY Security failed: routeScanCanopies=${security.routeScanCanopies || 0}/1`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('security');
  if (!frame) {
    failures.push('Gate 4-E-BY Security failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-BY Security failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-BY Security failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-BY Security failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 28 || frame.approachDistance > 44) {
    failures.push(`Gate 4-E-BY Security failed: approachDistance=${frame.approachDistance}`);
  }
...
function assertGate4ECAProjectsRouteFoundryFacadeBenchmarkVerification(result, failures) {
  assertGate4EBZRouteFramingBenchmarkSweepVerification(result, failures);

  const projects = result.gate4b4?.projects || {};
  assertAuthoredDistrictAsset(result, 'EnvPolishProjectsFoundryBuilding', 'Gate 4-E-CA Projects route foundry facade architecture', failures);
  if ((projects.routeRepositoryAtriumFaces || 0) < 1) {
    failures.push(`Gate 4-E-CA Projects failed: routeRepositoryAtriumFaces=${projects.routeRepositoryAtriumFaces || 0}/1`);
  }
  if ((projects.routeBuildPipelineRunways || 0) < 1) {
    failures.push(`Gate 4-E-CA Projects failed: routeBuildPipelineRunways=${projects.routeBuildPipelineRunways || 0}/1`);
  }
  if ((projects.routePrototypeReviewWindows || 0) < 3) {
    failures.push(`Gate 4-E-CA Projects failed: routePrototypeReviewWindows=${projects.routePrototypeReviewWindows || 0}/3`);
  }
  if ((projects.routeReleaseCraneCrowns || 0) < 1) {
    failures.push(`Gate 4-E-CA Projects failed: routeReleaseCraneCrowns=${projects.routeReleaseCraneCrowns || 0}/1`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('projects');
  if (!frame) {
    failures.push('Gate 4-E-CA Projects failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-CA Projects failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-CA Projects failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-CA Projects failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!frame.road?.offsetSafe || Math.abs(frame.road?.lateralOffset || 0) > 0.85) {
    failures.push(`Gate 4-E-CA Projects failed: road lateral=${frame.road?.lateralOffset}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 16 || frame.approachDistance > 46) {
    failures.push(`Gate 4-E-CA Projects failed: approachDistance=${frame.approachDistance}`);
  }
}

function assertGate4ECBSkillsRouteLearningSystemsFacadeBenchmarkVerification(result, failures) {
  assertGate4ECAProjectsRouteFoundryFacadeBenchmarkVerification(result, failures);

  const skills = result.gate4b2?.skills || {};
  assertAuthoredDistrictAsset(result, 'EnvPolishSkillsDataCenter', 'Gate 4-E-CB Skills route learning-systems facade architecture', failures);
  if ((skills.routeLearningFacadeFaces || 0) < 2) {
    failures.push(`Gate 4-E-CB Skills failed: routeLearningFacadeFaces=${skills.routeLearningFacadeFaces || 0}/2`);
  }
  if ((skills.routeCurriculumRibbons || 0) < 6) {
    failures.push(`Gate 4-E-CB Skills failed: routeCurriculumRibbons=${skills.routeCurriculumRibbons || 0}/6`);
  }
  if ((skills.routePracticeLabWindows || 0) < 6) {
    failures.push(`Gate 4-E-CB Skills failed: routePracticeLabWindows=${skills.routePracticeLabWindows || 0}/6`);
  }
  if ((skills.routeSkillBeaconCrowns || 0) < 1) {
    failures.push(`Gate 4-E-CB Skills failed: routeSkillBeaconCrowns=${skills.routeSkillBeaconCrowns || 0}/1`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('skills');
  if (!frame) {
    failures.push('Gate 4-E-CB Skills failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-CB Skills failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-CB Skills failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-CB Skills failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!frame.road?.offsetSafe || Math.abs(frame.road?.lateralOffset || 0) > 0.85) {
    failures.push(`Gate 4-E-CB Skills failed: road lateral=${frame.road?.lateralOffset}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 16 || frame.approachDistance > 46) {
...
function assertGate4ECELaunchHubForecourtFirstFrameBenchmarkVerification(result, failures) {
  assertGate4ECDPotatoRouteHarvestSilhouetteBenchmarkVerification(result, failures);

  const launch = result.gate4eLaunchHub || {};
  assertAuthoredDistrictAsset(result, 'EnvPolishLaunchHubGateway', 'Gate 4-E-CE Launch Hub forecourt first-frame architecture', failures);
  if ((launch.firstFrameForecourts || 0) < 1) {
    failures.push(`Gate 4-E-CE Launch Hub failed: firstFrameForecourts=${launch.firstFrameForecourts || 0}/1`);
  }
  if ((launch.routeThresholdCanopies || 0) < 1) {
    failures.push(`Gate 4-E-CE Launch Hub failed: routeThresholdCanopies=${launch.routeThresholdCanopies || 0}/1`);
  }
  if ((launch.forecourtSideWalls || 0) < 2) {
    failures.push(`Gate 4-E-CE Launch Hub failed: forecourtSideWalls=${launch.forecourtSideWalls || 0}/2`);
  }
  if ((launch.destinationBandCues || 0) < 6) {
    failures.push(`Gate 4-E-CE Launch Hub failed: destinationBandCues=${launch.destinationBandCues || 0}/6`);
  }
  if ((launch.supportFrames || 0) !== 0) {
    failures.push(`Gate 4-E-CE Launch Hub failed: loose supportFrames=${launch.supportFrames || 0}`);
  }
  if ((launch.guideTiles || 0) !== 0) {
    failures.push(`Gate 4-E-CE Launch Hub failed: guideTiles=${launch.guideTiles || 0}`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('landing');
  if (!frame) {
    failures.push('Gate 4-E-CE Launch Hub failed: missing landing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-CE Launch Hub failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-CE Launch Hub failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-CE Launch Hub failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!frame.road?.offsetSafe || Math.abs(frame.road?.lateralOffset || 0) > 0.85) {
    failures.push(`Gate 4-E-CE Launch Hub failed: road lateral=${frame.road?.lateralOffset}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 14 || frame.approachDistance > 38) {
    failures.push(`Gate 4-E-CE Launch Hub failed: approachDistance=${frame.approachDistance}`);
  }
...
function assertGate4ECISecurityOperationsThresholdBenchmarkVerification(result, failures) {
  assertGate4ECHSkillsSideLearningCampusBenchmarkVerification(result, failures);

  if (!result.securityScan?.active?.active) failures.push('Gate 4-E-CI Security failed: scan active state not observed');
  if (!result.securityScan?.complete?.complete) failures.push('Gate 4-E-CI Security failed: scan complete state not observed');

  const security = result.securityLab || {};
  assertAuthoredDistrictAsset(result, 'EnvPolishSecurityOperationsGate', 'Gate 4-E-CI Security operations threshold architecture', failures);
  if ((security.routeOperationsThresholds || 0) < 1) {
    failures.push(`Gate 4-E-CI Security failed: routeOperationsThresholds=${security.routeOperationsThresholds || 0}/1`);
  }
  if ((security.routeScanCanopyBridges || 0) < 1) {
    failures.push(`Gate 4-E-CI Security failed: routeScanCanopyBridges=${security.routeScanCanopyBridges || 0}/1`);
  }
  if ((security.routeThreatReviewGalleries || 0) < 1) {
    failures.push(`Gate 4-E-CI Security failed: routeThreatReviewGalleries=${security.routeThreatReviewGalleries || 0}/1`);
  }
  if ((security.routeAccessReviewBays || 0) < 4) {
    failures.push(`Gate 4-E-CI Security failed: routeAccessReviewBays=${security.routeAccessReviewBays || 0}/4`);
  }
  if ((security.routeScanStatusBands || 0) < 6) {
    failures.push(`Gate 4-E-CI Security failed: routeScanStatusBands=${security.routeScanStatusBands || 0}/6`);
  }
  if ((security.operationsGates || 0) < 1) {
    failures.push(`Gate 4-E-CI Security carry-forward failed: operationsGates=${security.operationsGates || 0}/1`);
  }
  if ((security.routeScanCanopies || 0) < 1) {
    failures.push(`Gate 4-E-CI Security carry-forward failed: routeScanCanopies=${security.routeScanCanopies || 0}/1`);
  }
  if ((security.routeCampusArrivalArches || 0) < 1) {
    failures.push(`Gate 4-E-CI Security carry-forward failed: routeCampusArrivalArches=${security.routeCampusArrivalArches || 0}/1`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('security');
  if (!frame) {
    failures.push('Gate 4-E-CI Security failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-CI Security failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-CI Security failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-CI Security failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!frame.road?.offsetSafe || Math.abs(frame.road?.lateralOffset || 0) > 0.85) {
    failures.push(`Gate 4-E-CI Security failed: road lateral=${frame.road?.lateralOffset}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 20 || frame.approachDistance > 46) {
    failures.push(`Gate 4-E-CI Security failed: approachDistance=${frame.approachDistance}`);
  }
  if ((result.colliderCount || 0) !== 2) failures.push(`Gate 4-E-CI failed: unexpected collider count=${result.colliderCount || 0}`);
  if ((result.forwardDriveProbe?.halts || 0) !== 0) failures.push(`Gate 4-E-CI failed: forwardDriveProbe.halts=${result.forwardDriveProbe?.halts || 0}`);
}

function assertGate4ECJCircuitDriveThroughCheckpointBenchmarkVerification(result, failures) {
  assertGate4ECISecurityOperationsThresholdBenchmarkVerification(result, failures);

  const circuit = result.gate4b5?.circuit || {};
  assertAuthoredDistrictAsset(result, 'EnvPolishCircuitTimeTrialGate', 'Gate 4-E-CJ Circuit drive-through checkpoint architecture', failures);
  if ((circuit.routeStarterArcades || 0) < 1) {
    failures.push(`Gate 4-E-CJ Circuit failed: routeStarterArcades=${circuit.routeStarterArcades || 0}/1`);
  }
  if ((circuit.routeSplitTimerBlades || 0) < 1) {
    failures.push(`Gate 4-E-CJ Circuit failed: routeSplitTimerBlades=${circuit.routeSplitTimerBlades || 0}/1`);
  }
  if ((circuit.routeCountdownTrees || 0) < 1) {
    failures.push(`Gate 4-E-CJ Circuit failed: routeCountdownTrees=${circuit.routeCountdownTrees || 0}/1`);
  }
  if ((circuit.routeCheckeredCrowns || 0) < 1) {
    failures.push(`Gate 4-E-CJ Circuit failed: routeCheckeredCrowns=${circuit.routeCheckeredCrowns || 0}/1`);
  }
  if ((circuit.routeRecoveryRailRuns || 0) < 2) {
    failures.push(`Gate 4-E-CJ Circuit failed: routeRecoveryRailRuns=${circuit.routeRecoveryRailRuns || 0}/2`);
  }
  if ((circuit.routeApproachTimingPortals || 0) < 1) {
    failures.push(`Gate 4-E-CJ Circuit carry-forward failed: routeApproachTimingPortals=${circuit.routeApproachTimingPortals || 0}/1`);
  }
  if ((circuit.routeApproachCheckpointLaneFrames || 0) < 3) {
    failures.push(`Gate 4-E-CJ Circuit carry-forward failed: routeApproachCheckpointLaneFrames=${circuit.routeApproachCheckpointLaneFrames || 0}/3`);
  }
  if (!result.circuit?.preview?.active || (result.circuit?.preview?.targetCount || 0) < 12) {
    failures.push(`Gate 4-E-CJ Circuit failed: preview active=${result.circuit?.preview?.active} targetCount=${result.circuit?.preview?.targetCount || 0}`);
  }
  if (!/Circuit complete/.test(result.circuit?.uiAfterFinish?.text || '')) {
    failures.push(`Gate 4-E-CJ Circuit failed: finish text=${result.circuit?.uiAfterFinish?.text || ''}`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('circuit');
  if (!frame) {
    failures.push('Gate 4-E-CJ Circuit failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-CJ Circuit failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-CJ Circuit failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-CJ Circuit failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!frame.road?.offsetSafe || Math.abs(frame.road?.lateralOffset || 0) > 0.85) {
    failures.push(`Gate 4-E-CJ Circuit failed: road lateral=${frame.road?.lateralOffset}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 16 || frame.approachDistance > 46) {
    failures.push(`Gate 4-E-CJ Circuit failed: approachDistance=${frame.approachDistance}`);
  }
  if ((result.colliderCount || 0) !== 2) failures.push(`Gate 4-E-CJ failed: unexpected collider count=${result.colliderCount || 0}`);
  if ((result.forwardDriveProbe?.halts || 0) !== 0) failures.push(`Gate 4-E-CJ failed: forwardDriveProbe.halts=${result.forwardDriveProbe?.halts || 0}`);
}

function assertGate4ECKAwardsRouteHonorsArrivalBenchmarkVerification(result, failures) {
  assertGate4ECJCircuitDriveThroughCheckpointBenchmarkVerification(result, failures);

  const awards = result.gate4b5?.awards || {};
  assertAuthoredDistrictAsset(result, 'EnvPolishAwardsMuseumHall', 'Gate 4-E-CK Awards honors arrival architecture', failures);
  if ((awards.routeHonorsArrivalArches || 0) < 1) {
    failures.push(`Gate 4-E-CK Awards failed: routeHonorsArrivalArches=${awards.routeHonorsArrivalArches || 0}/1`);
  }
  if ((awards.routeHonorsMedalWindows || 0) < 1) {
    failures.push(`Gate 4-E-CK Awards failed: routeHonorsMedalWindows=${awards.routeHonorsMedalWindows || 0}/1`);
  }
  if ((awards.routeCertificateFriezes || 0) < 2) {
    failures.push(`Gate 4-E-CK Awards failed: routeCertificateFriezes=${awards.routeCertificateFriezes || 0}/2`);
  }
  if ((awards.routeHonorsForecourts || 0) < 1) {
    failures.push(`Gate 4-E-CK Awards carry-forward failed: routeHonorsForecourts=${awards.routeHonorsForecourts || 0}/1`);
  }
  if ((awards.routeMedalGates || 0) < 1) {
    failures.push(`Gate 4-E-CK Awards carry-forward failed: routeMedalGates=${awards.routeMedalGates || 0}/1`);
  }
  if ((awards.routeTrophyRotundas || 0) < 1) {
    failures.push(`Gate 4-E-CK Awards carry-forward failed: routeTrophyRotundas=${awards.routeTrophyRotundas || 0}/1`);
  }
  if ((awards.signs || 0) !== 0) {
    failures.push(`Gate 4-E-CK Awards failed: rejected signs=${awards.signs || 0}`);
  }
  if ((awards.lamps || 0) !== 0) {
    failures.push(`Gate 4-E-CK Awards failed: rejected lamps=${awards.lamps || 0}`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('awards');
  if (!frame) {
    failures.push('Gate 4-E-CK Awards failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-CK Awards failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-CK Awards failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-CK Awards failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!frame.road?.offsetSafe || Math.abs(frame.road?.lateralOffset || 0) > 0.85) {
    failures.push(`Gate 4-E-CK Awards failed: road lateral=${frame.road?.lateralOffset}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 20 || frame.approachDistance > 46) {
    failures.push(`Gate 4-E-CK Awards failed: approachDistance=${frame.approachDistance}`);
  }
  if ((result.colliderCount || 0) !== 2) failures.push(`Gate 4-E-CK failed: unexpected collider count=${result.colliderCount || 0}`);
  if ((result.forwardDriveProbe?.halts || 0) !== 0) failures.push(`Gate 4-E-CK failed: forwardDriveProbe.halts=${result.forwardDriveProbe?.halts || 0}`);
}

function assertGate4ECLLaunchHubWorldEntryFirstFrameBenchmarkVerification(result, failures) {
  assertGate4ECKAwardsRouteHonorsArrivalBenchmarkVerification(result, failures);

  const launch = result.gate4eLaunchHub || {};
  assertAuthoredDistrictAsset(result, 'EnvPolishLaunchHubGateway', 'Gate 4-E-CL Launch Hub world-entry first-frame architecture', failures);
  if ((launch.worldEntryPortals || 0) < 1) {
    failures.push(`Gate 4-E-CL Launch Hub failed: worldEntryPortals=${launch.worldEntryPortals || 0}/1`);
  }
  if ((launch.routeEntrySignalFrames || 0) < 1) {
    failures.push(`Gate 4-E-CL Launch Hub failed: routeEntrySignalFrames=${launch.routeEntrySignalFrames || 0}/1`);
  }
  if ((launch.portfolioThresholdPanels || 0) < 3) {
    failures.push(`Gate 4-E-CL Launch Hub failed: portfolioThresholdPanels=${launch.portfolioThresholdPanels || 0}/3`);
  }
  if ((launch.firstFrameForecourts || 0) < 1) {
    failures.push(`Gate 4-E-CL Launch Hub carry-forward failed: firstFrameForecourts=${launch.firstFrameForecourts || 0}/1`);
  }
  if ((launch.routeThresholdCanopies || 0) < 1) {
    failures.push(`Gate 4-E-CL Launch Hub carry-forward failed: routeThresholdCanopies=${launch.routeThresholdCanopies || 0}/1`);
  }
  if ((launch.forecourtSideWalls || 0) < 2) {
    failures.push(`Gate 4-E-CL Launch Hub carry-forward failed: forecourtSideWalls=${launch.forecourtSideWalls || 0}/2`);
  }
  if ((launch.supportFrames || 0) !== 0) {
    failures.push(`Gate 4-E-CL Launch Hub failed: loose supportFrames=${launch.supportFrames || 0}`);
  }
  if ((launch.guideTiles || 0) !== 0) {
    failures.push(`Gate 4-E-CL Launch Hub failed: guideTiles=${launch.guideTiles || 0}`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('landing');
  if (!frame) {
    failures.push('Gate 4-E-CL Launch Hub failed: missing landing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-CL Launch Hub failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-CL Launch Hub failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-CL Launch Hub failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!frame.road?.offsetSafe || Math.abs(frame.road?.lateralOffset || 0) > 0.85) {
    failures.push(`Gate 4-E-CL Launch Hub failed: road lateral=${frame.road?.lateralOffset}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 14 || frame.approachDistance > 38) {
    failures.push(`Gate 4-E-CL Launch Hub failed: approachDistance=${frame.approachDistance}`);
  }
...
function assertGate4ECNContactPublicSignalArrivalBenchmarkVerification(result, failures) {
  assertGate4ECMProjectsPublicBuildForecourtBenchmarkVerification(result, failures);

  const harbor = result.gate4b4?.harbor || {};
  assertAuthoredDistrictAsset(result, 'EnvPolishSignalHarborCommunicationsStation', 'Gate 4-E-CN Contact public signal arrival architecture', failures);
  if ((harbor.routePublicSignalArrivalThresholds || 0) < 1) {
    failures.push(`Gate 4-E-CN Contact failed: routePublicSignalArrivalThresholds=${harbor.routePublicSignalArrivalThresholds || 0}/1`);
  }
  if ((harbor.routeVehicleMessagePortals || 0) < 1) {
    failures.push(`Gate 4-E-CN Contact failed: routeVehicleMessagePortals=${harbor.routeVehicleMessagePortals || 0}/1`);
  }
  if ((harbor.routeContactSignalWindows || 0) < 1) {
    failures.push(`Gate 4-E-CN Contact failed: routeContactSignalWindows=${harbor.routeContactSignalWindows || 0}/1`);
  }
  if ((harbor.routeMessageArrivalBays || 0) < 3) {
    failures.push(`Gate 4-E-CN Contact failed: routeMessageArrivalBays=${harbor.routeMessageArrivalBays || 0}/3`);
  }
  if ((harbor.routeContactForecourts || 0) < 1) {
    failures.push(`Gate 4-E-CN Contact carry-forward failed: routeContactForecourts=${harbor.routeContactForecourts || 0}/1`);
  }
  if ((harbor.routeContactArrivalConcourses || 0) < 1) {
    failures.push(`Gate 4-E-CN Contact carry-forward failed: routeContactArrivalConcourses=${harbor.routeContactArrivalConcourses || 0}/1`);
  }
  if ((harbor.routePublicSignalGateways || 0) < 1) {
    failures.push(`Gate 4-E-CN Contact carry-forward failed: routePublicSignalGateways=${harbor.routePublicSignalGateways || 0}/1`);
  }
  if ((harbor.signs || 0) !== 0) {
    failures.push(`Gate 4-E-CN Contact failed: rejected signs=${harbor.signs || 0}`);
  }
  if ((harbor.lamps || 0) !== 0) {
    failures.push(`Gate 4-E-CN Contact failed: rejected lamps=${harbor.lamps || 0}`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('contact');
  if (!frame) {
    failures.push('Gate 4-E-CN Contact failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-CN Contact failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-CN Contact failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-CN Contact failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!frame.road?.offsetSafe || Math.abs(frame.road?.lateralOffset || 0) > 0.85) {
    failures.push(`Gate 4-E-CN Contact failed: road lateral=${frame.road?.lateralOffset}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 20 || frame.approachDistance > 42) {
    failures.push(`Gate 4-E-CN Contact failed: approachDistance=${frame.approachDistance}`);
  }
  if ((result.colliderCount || 0) !== 2) failures.push(`Gate 4-E-CN failed: unexpected collider count=${result.colliderCount || 0}`);
  if ((result.forwardDriveProbe?.halts || 0) !== 0) failures.push(`Gate 4-E-CN failed: forwardDriveProbe.halts=${result.forwardDriveProbe?.halts || 0}`);
}

function assertGate4ECOProjectsHierarchySimplificationBenchmarkVerification(result, failures) {
  assertGate4ECNContactPublicSignalArrivalBenchmarkVerification(result, failures);

  const projects = result.gate4b4?.projects || {};
  assertAuthoredDistrictAsset(result, 'EnvPolishProjectsFoundryBuilding', 'Gate 4-E-CO Projects hierarchy simplification architecture', failures);
  if ((projects.routePublicBuildForecourts || 0) < 1) {
    failures.push(`Gate 4-E-CO Projects failed: routePublicBuildForecourts=${projects.routePublicBuildForecourts || 0}/1`);
  }
  if ((projects.routeVehicleScaleBuildArches || 0) < 1) {
    failures.push(`Gate 4-E-CO Projects failed: routeVehicleScaleBuildArches=${projects.routeVehicleScaleBuildArches || 0}/1`);
  }
  if ((projects.routeOpenWorkshopDoors || 0) < 1) {
    failures.push(`Gate 4-E-CO Projects failed: routeOpenWorkshopDoors=${projects.routeOpenWorkshopDoors || 0}/1`);
  }
  if ((projects.routeReleaseReviewBays || 0) < 3) {
    failures.push(`Gate 4-E-CO Projects failed: routeReleaseReviewBays=${projects.routeReleaseReviewBays || 0}/3`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('projects');
  if (!frame) {
    failures.push('Gate 4-E-CO Projects failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-CO Projects failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-CO Projects failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-CO Projects failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!frame.road?.offsetSafe || Math.abs(frame.road?.lateralOffset || 0) > 0.85) {
    failures.push(`Gate 4-E-CO Projects failed: road lateral=${frame.road?.lateralOffset}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 16 || frame.approachDistance > 46) {
    failures.push(`Gate 4-E-CO Projects failed: approachDistance=${frame.approachDistance}`);
  }
  if ((result.highQuality?.triangles || 0) > 329986) {
    failures.push(`Gate 4-E-CO Projects failed: high-quality triangles grew over CN baseline (${result.highQuality?.triangles || 0}/329986)`);
  }
  if ((result.colliderCount || 0) !== 2) failures.push(`Gate 4-E-CO failed: unexpected collider count=${result.colliderCount || 0}`);
  if ((result.forwardDriveProbe?.halts || 0) !== 0) failures.push(`Gate 4-E-CO failed: forwardDriveProbe.halts=${result.forwardDriveProbe?.halts || 0}`);
}

function assertGate4ECPProjectsBuildHallSilhouetteBenchmarkVerification(result, failures) {
  assertGate4ECOProjectsHierarchySimplificationBenchmarkVerification(result, failures);

  const projects = result.gate4b4?.projects || {};
  assertAuthoredDistrictAsset(result, 'EnvPolishProjectsFoundryBuilding', 'Gate 4-E-CP Projects build hall silhouette architecture', failures);
  if ((projects.routePublicBuildForecourts || 0) < 1) {
    failures.push(`Gate 4-E-CP Projects failed: routePublicBuildForecourts=${projects.routePublicBuildForecourts || 0}/1`);
  }
  if ((projects.routeVehicleScaleBuildArches || 0) < 1) {
    failures.push(`Gate 4-E-CP Projects failed: routeVehicleScaleBuildArches=${projects.routeVehicleScaleBuildArches || 0}/1`);
  }
  if ((projects.routeOpenWorkshopDoors || 0) < 1) {
    failures.push(`Gate 4-E-CP Projects failed: routeOpenWorkshopDoors=${projects.routeOpenWorkshopDoors || 0}/1`);
  }
  if ((projects.routeReleaseReviewBays || 0) < 3) {
    failures.push(`Gate 4-E-CP Projects failed: routeReleaseReviewBays=${projects.routeReleaseReviewBays || 0}/3`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('projects');
  if (!frame) {
    failures.push('Gate 4-E-CP Projects failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-CP Projects failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-CP Projects failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-CP Projects failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!frame.road?.offsetSafe || Math.abs(frame.road?.lateralOffset || 0) > 0.85) {
    failures.push(`Gate 4-E-CP Projects failed: road lateral=${frame.road?.lateralOffset}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 16 || frame.approachDistance > 46) {
    failures.push(`Gate 4-E-CP Projects failed: approachDistance=${frame.approachDistance}`);
  }
  if ((result.highQuality?.triangles || 0) > 327994) {
    failures.push(`Gate 4-E-CP Projects failed: high-quality triangles grew over CO baseline (${result.highQuality?.triangles || 0}/327994)`);
  }
  if ((result.colliderCount || 0) !== 2) failures.push(`Gate 4-E-CP failed: unexpected collider count=${result.colliderCount || 0}`);
  if ((result.forwardDriveProbe?.halts || 0) !== 0) failures.push(`Gate 4-E-CP failed: forwardDriveProbe.halts=${result.forwardDriveProbe?.halts || 0}`);
}

function assertGate4ECQProjectsSingleHallFacadeBenchmarkVerification(result, failures) {
  assertGate4ECPProjectsBuildHallSilhouetteBenchmarkVerification(result, failures);

  const projects = result.gate4b4?.projects || {};
  assertAuthoredDistrictAsset(result, 'EnvPolishProjectsFoundryBuilding', 'Gate 4-E-CQ Projects single-hall facade architecture', failures);
  if ((projects.routePublicBuildForecourts || 0) < 1) {
    failures.push(`Gate 4-E-CQ Projects failed: routePublicBuildForecourts=${projects.routePublicBuildForecourts || 0}/1`);
  }
  if ((projects.routeVehicleScaleBuildArches || 0) < 1) {
    failures.push(`Gate 4-E-CQ Projects failed: routeVehicleScaleBuildArches=${projects.routeVehicleScaleBuildArches || 0}/1`);
  }
  if ((projects.routeOpenWorkshopDoors || 0) < 1) {
    failures.push(`Gate 4-E-CQ Projects failed: routeOpenWorkshopDoors=${projects.routeOpenWorkshopDoors || 0}/1`);
  }
  if ((projects.routeReleaseReviewBays || 0) < 3) {
    failures.push(`Gate 4-E-CQ Projects failed: routeReleaseReviewBays=${projects.routeReleaseReviewBays || 0}/3`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('projects');
  if (!frame) {
    failures.push('Gate 4-E-CQ Projects failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-CQ Projects failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-CQ Projects failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-CQ Projects failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!frame.road?.offsetSafe || Math.abs(frame.road?.lateralOffset || 0) > 0.85) {
    failures.push(`Gate 4-E-CQ Projects failed: road lateral=${frame.road?.lateralOffset}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 16 || frame.approachDistance > 46) {
    failures.push(`Gate 4-E-CQ Projects failed: approachDistance=${frame.approachDistance}`);
  }
  if ((result.highQuality?.triangles || 0) > 327642) {
    failures.push(`Gate 4-E-CQ Projects failed: high-quality triangles grew over CP baseline (${result.highQuality?.triangles || 0}/327642)`);
  }
  if ((result.colliderCount || 0) !== 2) failures.push(`Gate 4-E-CQ failed: unexpected collider count=${result.colliderCount || 0}`);
  if ((result.forwardDriveProbe?.halts || 0) !== 0) failures.push(`Gate 4-E-CQ failed: forwardDriveProbe.halts=${result.forwardDriveProbe?.halts || 0}`);
}

function assertGate4ECRProjectsMaterialSilhouetteBenchmarkVerification(result, failures) {
  assertGate4ECQProjectsSingleHallFacadeBenchmarkVerification(result, failures);

  const polishPropsBytes = result.glbAssets?.['play/game-assets/polish-props.glb'] || 0;
  if (polishPropsBytes > 8602264) {
    failures.push(`Gate 4-E-CR Projects failed: polish-props.glb grew over CQ baseline (${polishPropsBytes}/8602264)`);
  }
  if ((result.highQuality?.triangles || 0) > 327642) {
    failures.push(`Gate 4-E-CR Projects failed: high-quality triangles grew over CQ baseline (${result.highQuality?.triangles || 0}/327642)`);
  }
  if ((result.highQuality?.calls || 0) > 306) {
    failures.push(`Gate 4-E-CR Projects failed: high-quality draw calls grew over CQ baseline (${result.highQuality?.calls || 0}/306)`);
  }
}

function assertGate4ECSAwardsSolidEntryBenchmarkVerification(result, failures) {
  assertGate4ECRProjectsMaterialSilhouetteBenchmarkVerification(result, failures);

  const awards = result.gate4b5?.awards || {};
  assertAuthoredDistrictAsset(result, 'EnvPolishAwardsMuseumHall', 'Gate 4-E-CS Awards solid honors entry architecture', failures);
  if ((awards.routeHonorsEntryHalls || 0) < 1) {
    failures.push(`Gate 4-E-CS Awards failed: routeHonorsEntryHalls=${awards.routeHonorsEntryHalls || 0}/1`);
  }
  if ((awards.routeSolidHonorsFacades || 0) < 1) {
    failures.push(`Gate 4-E-CS Awards failed: routeSolidHonorsFacades=${awards.routeSolidHonorsFacades || 0}/1`);
  }
  if ((awards.routeMedalWindowBackplates || 0) < 1) {
    failures.push(`Gate 4-E-CS Awards failed: routeMedalWindowBackplates=${awards.routeMedalWindowBackplates || 0}/1`);
  }

  const frames = new Map((result.routeApproachFraming || []).map((frame) => [frame.id, frame]));
  const frame = frames.get('awards');
  if (!frame) {
    failures.push('Gate 4-E-CS Awards failed: missing route approach frame');
    return;
  }
  if (!frame.safeInFrame) failures.push(`Gate 4-E-CS Awards failed: safeInFrame=${frame.safeInFrame}`);
  if (!frame.vehicle?.safeInFrame) {
    failures.push(`Gate 4-E-CS Awards failed: vehicle frame=${JSON.stringify(frame.vehicle?.bounds || {})}`);
  }
  if (!frame.landmark?.inFrame || !frame.landmark?.safeInFrame) {
    failures.push(`Gate 4-E-CS Awards failed: landmark frame=${JSON.stringify(frame.landmark?.bounds || {})}`);
  }
  if (!frame.road?.offsetSafe || Math.abs(frame.road?.lateralOffset || 0) > 0.85) {
    failures.push(`Gate 4-E-CS Awards failed: road lateral=${frame.road?.lateralOffset}`);
  }
  if (!Number.isFinite(frame.approachDistance) || frame.approachDistance < 20 || frame.approachDistance > 46) {
    failures.push(`Gate 4-E-CS Awards failed: approachDistance=${frame.approachDistance}`);
  }

  const polishPropsBytes = result.glbAssets?.['play/game-assets/polish-props.glb'] || 0;
  if (polishPropsBytes > 8602264) {
    failures.push(`Gate 4-E-CS Awards failed: polish-props.glb grew over CQ benchmark (${polishPropsBytes}/8602264)`);
  }
  if ((result.highQuality?.triangles || 0) > 327642) {
    failures.push(`Gate 4-E-CS Awards failed: high-quality triangles grew over CQ benchmark (${result.highQuality?.triangles || 0}/327642)`);
  }
...
function assertGate4FRCLandmarkReplacementVerification(result, failures) {
  ...
  const sentinelAsset = (result.authoredDistrictAssets || []).find((entry) => entry.name === 'EnvPolishSentinelSocTower');
  if (!sentinelAsset?.template) failures.push('Gate 4-FR-C Sentinel replacement failed: EnvPolishSentinelSocTower template missing');
  if (!sentinelAsset?.placed) failures.push('Gate 4-FR-C Sentinel replacement failed: EnvPolishSentinelSocTower not placed');
  const awardsAsset = (result.authoredDistrictAssets || []).find((entry) => entry.name === 'EnvPolishAwardsMuseumHall');
  if (!awardsAsset?.template) failures.push('Gate 4-FR-C Awards replacement failed: EnvPolishAwardsMuseumHall template missing');
  if (!awardsAsset?.placed) failures.push('Gate 4-FR-C Awards replacement failed: EnvPolishAwardsMuseumHall not placed');
  const todoAsset = (result.authoredDistrictAssets || []).find((entry) => entry.name === 'EnvPolishTodoPlanningStudio');
  if (!todoAsset?.template) failures.push('Gate 4-FR-C Todo replacement failed: EnvPolishTodoPlanningStudio template missing');
  if (!todoAsset?.placed) failures.push('Gate 4-FR-C Todo replacement failed: EnvPolishTodoPlanningStudio not placed');
  const potatoAsset = (result.authoredDistrictAssets || []).find((entry) => entry.name === 'EnvPolishPotatoFarmStand');
  if (!potatoAsset?.template) failures.push('Gate 4-FR-C Potato replacement failed: EnvPolishPotatoFarmStand template missing');
  if (!potatoAsset?.placed) failures.push('Gate 4-FR-C Potato replacement failed: EnvPolishPotatoFarmStand not placed');
  ...
  const careerInspection = result.gate4frCCareerInspection || {};
  ...
  const contactInspection = result.gate4frCContactInspection || {};
  ...
  const sentinelInspection = result.gate4frCSentinelInspection || {};
  if (!sentinelInspection.enabled) failures.push('Gate 4-FR-C Sentinel inspection failed: 360 screenshot pass not enabled');
  if ((sentinelInspection.screenshots || []).length !== 7) {
    failures.push(`Gate 4-FR-C Sentinel inspection failed: screenshots=${(sentinelInspection.screenshots || []).length}/7`);
  }
  const awardsInspection = result.gate4frCAwardsInspection || {};
  if (!awardsInspection.enabled) failures.push('Gate 4-FR-C Awards inspection failed: 360 screenshot pass not enabled');
  if ((awardsInspection.screenshots || []).length !== 7) {
    failures.push(`Gate 4-FR-C Awards inspection failed: screenshots=${(awardsInspection.screenshots || []).length}/7`);
  }
  const todoInspection = result.gate4frCTodoInspection || {};
  if (!todoInspection.enabled) failures.push('Gate 4-FR-C Todo inspection failed: 360 screenshot pass not enabled');
  if ((todoInspection.screenshots || []).length !== 7) {
    failures.push(`Gate 4-FR-C Todo inspection failed: screenshots=${(todoInspection.screenshots || []).length}/7`);
  }
  const potatoInspection = result.gate4frCPotatoInspection || {};
  if (!potatoInspection.enabled) failures.push('Gate 4-FR-C Potato inspection failed: 360 screenshot pass not enabled');
