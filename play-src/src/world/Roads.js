...
export class Roads {
  ...
  addSegment(a, b, path) {
    ...
    const shoulder = new THREE.Mesh(
      new THREE.BoxGeometry(width + style.shoulder * 2, 0.12, length + width * 0.45),
      this.world.materials.roadEdge
    );
    shoulder.name = `ROAD_${path.id}_shoulder`;
    shoulder.position.set(x, 0.055, z);
    shoulder.rotation.y = rotation;
    shoulder.receiveShadow = true;
    this.world.scene.add(shoulder);

    const stone = new THREE.Mesh(
      new THREE.BoxGeometry(width, 0.16, length + width * 0.28),
    ...
    for (let i = 0; i < dashCount; i += 1) {
      ...
      dash.position.set(x + Math.sin(rotation) * length * t, 0.215, z + Math.cos(rotation) * length * t);
  ...
  addNode(point, path) {
    ...
    const node = new THREE.Mesh(new THREE.CylinderGeometry(radius, radius, 0.13, 34), this.world.materials.roadEdge);
    node.name = `ROAD_${path.id}_node`;
    node.position.set(point[0], 0.07, point[1]);
    node.receiveShadow = true;
    this.world.scene.add(node);

    const top = new THREE.Mesh(new THREE.CylinderGeometry(radius - style.shoulder * 0.65, radius - style.shoulder * 0.65, 0.17, 34), this.world.materials.stoneRoad);
    top.position.set(point[0], 0.135, point[1]);
